...
from models.cctnet.checkpoint import load_checkpoint_beit, load_checkpoint
...
class BEiT(nn.Module):
    ...
    """
    def __init__(self, img_size=512, patch_size=16, in_chans=3, num_classes=6, embed_dim=768, depth=12,
                 num_heads=12, mlp_ratio=4., qkv_bias=True, qk_scale=None, drop_rate=0., attn_drop_rate=0.,
                 drop_path_rate=0.1, hybrid_backbone=None, norm_layer=None, init_values=0.1,
    ...
    def init_weights(self, pretrained=None, strict=False):
        ...
        if isinstance(pretrained, str):
            ...
            load_checkpoint_beit(self, pretrained, strict=strict)
...
class Beit(nn.Module):
    ...
    def __init__(self, nclass, img_size, patch_size, embed_dim, depth, num_heads,
                 head_dim, init_values, drop_path_rate, out_indices,
        ...
        self.backbone = BEiT(num_classes=nclass, img_size=img_size, patch_size=patch_size, embed_dim=embed_dim,
                             depth=depth, num_heads=num_heads, init_values=init_values,
                             drop_path_rate=drop_path_rate, out_indices=out_indices)
...
def beit_base(nclass, img_size, pretrained=False, aux=False, head='uperhead', edge_aux=False):
    if pretrained:
        pretrained_root = './pretrained_weights/beit_base_patch16_224_pt22k_ft22k.pth'
    else:
        pretrained_root = None
    model = Beit(nclass=nclass, img_size=img_size, patch_size=16, embed_dim=768, depth=12, num_heads=12,
                 head_dim=[768, 768, 768, 768], init_values=0.1, drop_path_rate=0.1, out_indices=[3, 5, 7, 11],
                 aux=aux, pretrained_root=pretrained_root, head=head, edge_aux=edge_aux)
    return model


def beit_large(nclass, img_size, pretrained=False, aux=False, head='uperhead', edge_aux=False):
    if pretrained:
        pretrained_root = './pretrained_weights/beit_large_patch16_224_pt22k_ft22k.pth'
    else:
        pretrained_root = None
    model = Beit(nclass=nclass, img_size=img_size, patch_size=16, embed_dim=1024, depth=24, num_heads=16,
                 head_dim=[1024, 1024, 1024, 1024], init_values=1e-6, drop_path_rate=0.2, out_indices=[7, 11, 15, 23],
                 aux=aux, pretrained_root=pretrained_root, head=head, edge_aux=edge_aux)
    return model
...
if __name__ == '__main__':
    ...
    model_base = beit_base(nclass=6, img_size=512, aux=True, edge_aux=False, head='uperhead', pretrained=False)
    # model_large = beit_large(nclass=6, img_size=512, aux=True, edge_aux=False, head='uperhead', pretrained=False)
