...
def _cfg(url='', **kwargs):
    return {
        'url': url,
        'nclass': 1000, 'input_size': (3, 224, 224), 'pool_size': None,
...
class CSWinTransformer(nn.Module):
    ...
    """
    def __init__(self, img_size=224, patch_size=16, in_chans=3, nclass=1000, embed_dim=96, depth=[2,2,6,2], split_size = [3,5,7],
                 num_heads=12, mlp_ratio=4., qkv_bias=True, qk_scale=None, drop_rate=0., attn_drop_rate=0.,
                 drop_path_rate=0., hybrid_backbone=None, norm_layer=nn.LayerNorm, use_chk=False,aux=False, pretrained_root=None, head='seghead', edge_aux=False):
        super().__init__()
        ...
        # Classifier head
        self.head = nn.Linear(curr_dim, nclass) if nclass > 0 else nn.Identity()
    ...
    def reset_classifier(self, nclass, global_pool=''):
        if self.nclass != nclass:
            print ('reset head to', nclass)
            self.nclass = nclass
            self.head = nn.Linear(self.out_dim, nclass) if nclass > 0 else nn.Identity()
...
@register_model
def CSWin_64_12211_tiny_224(pretrained=False, aux=False, head='uperhead', edge_aux=False,**kwargs):
    if pretrained:
        pretrained_root = '/root/workspace/data/ctb/cswin_tiny_224.pth'
    else:
        pretrained_root = None
    model = CSWinTransformer(patch_size=4, embed_dim=64, depth=[1,2,21,1],
        split_size=[1,2,7,7], num_heads=[2,4,8,16], mlp_ratio=4., aux=aux, head=head, edge_aux=edge_aux, **kwargs)
