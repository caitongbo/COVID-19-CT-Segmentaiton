...
class DaViT(nn.Module):
    ...
    def __init__(self, in_chans=3, depths=(1, 1, 3, 1), patch_size=4,
                 ...
                 ffn=True, overlapped_patch=False, cpe_act=False, weight_init='/root/workspace/data/ctb/COVID-19-CT/semi_ct/model_best.pth.tar',
        ...
        self.con1 = nn.Conv2d(96,2,1,1,1)
        self.con2 = nn.Conv2d(192,2,1,1,1)
        self.con3 = nn.Conv2d(384,2,1,1,1)
        self.con4 = nn.Conv2d(768,2,1,1,1)


    def init_weights(self, pretrained="/root/workspace/data/ctb/COVID-19-CT/semi_ct/model_best.pth2.tar"):
    ...
    def forward(self, x):
        ...
        outs[0] = self.con1(outs[0])
        outs[1] = self.con2(outs[1])
        outs[2] = self.con3(outs[2])
        outs[3] = self.con4(outs[3])

        sum = 0
        for i in outs:
            out = F.interpolate(i, (224, 224), mode='bilinear', align_corners=False)
            sum+=out
...
def davit(pretrained=False):
    if pretrained:
        pretrained_root = '/root/workspace/data/ctb/COVID-19-CT/semi_ct/model_best.pth.tar'
    else:
        pretrained_root = None
    model = DaViT(embed_dims=(96, 192, 384, 768),
        depths=(1, 1, 3, 1),
        num_heads=(3, 6, 12, 24),
        window_size=7,
        drop_path_rate=0.3,
    ...
    model.init_weights(pretrained=pretrained_root)
