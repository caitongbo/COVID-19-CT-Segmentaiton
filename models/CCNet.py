...
class ResNet(nn.Module):
    def __init__(self, num_classes, recurrence=1, block=Bottleneck, layers=[3, 4, 23, 3]):
        self.inplanes = 128
    ...
    def forward(self, x, labels=None):
        ...
        x = self.head(x_up2, self.recurrence)
        outs = [x, x_dsn]
...
def CCNet(num_classes=20, recurrence=2):
    model = ResNet(num_classes, recurrence)
    return model


if __name__ == '__main__':
    ras = CCNet(num_classes=20, recurrence=2).cuda()
