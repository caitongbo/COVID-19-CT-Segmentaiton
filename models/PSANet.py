...
import models.lib_psa.psa.functional as PF
# import model.resnet as models
from models.ResNet_PSANet import resnet101
...
class PSA(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.psa_type in [0, 1]:
            ...
            x = torch.matmul(x.view(n, c, h * w), y.view(n, h * w, h * w)).view(n, c, h, w) * (1.0 / self.normalization_factor)
            ...
            x_col = torch.matmul(x_col.view(n, c, h * w), y_col.view(n, h * w, h * w)).view(n, c, h, w) * (1.0 / self.normalization_factor)
            x_dis = torch.matmul(x_dis.view(n, c, h * w), y_dis.view(n, h * w, h * w)).view(n, c, h, w) * (1.0 / self.normalization_factor)
...
if __name__ == '__main__':
    # crop_h = crop_w = 880
    # input = torch.rand(1, 1, crop_h, crop_w).cuda()
    # compact = False
    ...
    #         pass
    #
    # model = PSANet(classes=20, mask_h=mask_h, mask_w=mask_w).cuda()
    # from torchsummary import summary
    #
    # print(summary(model, (1, 880, 880)))
    # out = model(input)
    # print(out.shape)
    ...
    # 导入模型，输入一张输入图片的尺寸
    model = PSANet(classes=20, mask_h=mask_h, mask_w=mask_w)
