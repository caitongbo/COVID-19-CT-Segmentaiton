...
class PyramidFeatures(nn.Module):
    def __init__(self, config, img_size = 224, in_channels=3):
        ...
        self.p1_ch = nn.Conv2d(256, config.swin_pyramid_fm[0] , kernel_size = 1)
    ...
    def forward(self, x):
        ...
        fm1_reshaped = Rearrange('b c h w -> b (h w) c')(fm1_ch) #cnn->trans         
        sw1 = self.swin_transformer.layers[0](fm1_reshaped)  #trans cac
        sw1_skipped = fm1_reshaped  + sw1  #trans residual
        norm1 = self.norm_1(sw1_skipped) #trans normal
        sw1_CLS = self.avgpool_1(norm1.transpose(1, 2)) #max
        sw1_CLS_reshaped = Rearrange('b c 1 -> b 1 c')(sw1_CLS) # channel
