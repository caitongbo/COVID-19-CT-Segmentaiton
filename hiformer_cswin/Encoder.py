...
class CSWinTransformer(nn.Module):
    ...
    """
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=96, depth=[2,2,6], split_size = [3,5,7],
...
class PyramidFeatures(nn.Module):
    def __init__(self, config, img_size = 224, in_channels=3):
        ...
        self.swin_transformer = CSWinTransformer(img_size,in_chans = 3,patch_size=4, embed_dim=64, depth=[1,2,21,1],
        ...
        self.norm_4 = nn.LayerNorm(config.swin_pyramid_fm[3])
        self.avgpool_4 = nn.AdaptiveAvgPool1d(1)    
    ...
    def forward(self, x):
        ...
        for blk in self.swin_transformer.stage1:
            sw1 = blk(fm1_reshaped)
        ...
        for blk in self.swin_transformer.stage2:
            fm1_sw2 = blk(fm1_sw1)
        fm2 = self.p2(fm1)
        ...
        for blk in self.swin_transformer.stage3:
            fm2_sw3 = blk(fm2_sw2)
        fm3 = self.p3(fm2)
        ...
        for blk in self.swin_transformer.stage4:
            fm3_sw4 = blk(fm3_sw3)
        fm4 = self.p4(fm3)
        ...
        norm4 = self.norm_4(fm4_sw4_skipped) 
        sw4_CLS = self.avgpool_4(norm4.transpose(1, 2))
        sw4_CLS_reshaped = Rearrange('b c 1 -> b 1 c')(sw4_CLS) 

        return [torch.cat((sw1_CLS_reshaped, sw1_skipped), dim=1), torch.cat((sw2_CLS_reshaped, fm2_sw2_skipped), dim=1), torch.cat((sw3_CLS_reshaped, fm3_sw3_skipped), dim=1), torch.cat((sw4_CLS_reshaped, fm4_sw4_skipped), dim=1)]

# DLF Module
class All2Cross(nn.Module):
    def __init__(self, config, img_size = 224 , in_chans=3, embed_dim=[64, 128, 256, 512], norm_layer=nn.LayerNorm):
        super().__init__()
        ...
        n_p2 = (config.image_size // config.patch_size // 2) ** 2  # default: 3136 
        n_p3 = (config.image_size // config.patch_size // 4) ** 2  # default: 3136 
        n_p4 = (config.image_size // config.patch_size // 8) ** 2  # default: 196 -->49
        num_patches = (n_p1, n_p2, n_p3, n_p4)
        self.num_branches = 4
