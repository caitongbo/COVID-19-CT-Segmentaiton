...
class CrossEntropyLoss2d(nn.Module):
    ...
    #     # return 1.0*loss1 + 1.0*loss2 + 0.5*loss3 + 0.5*loss4
    #     return 1.0*loss1 + 1.0*loss2 + loss3 + oss4
...
def main_tr(args, crossVal):
    ...
    trainDataset_main = myTransforms.Compose([
        ...
        myTransforms.RandomCropResize(1.0),
...
if __name__ == '__main__':
    ...
    parser.add_argument('--crossVal', type=int,  default=5, help='random seed')
...
#     args.width = 224
#     args.height = 224
# 'cctnet','TransUNet','ViTAdapter','UNeXt','FCN','UTNet','InfNet','MedT','SegNet',,'SwinUNet','UNet++','MiniSeg','PSPNet','AttUNet',
    # models = ['nnUNet','PSANet','BiSeNetv2','FPN']
    # 'ENet','GCN','ResUNet','ResUNetpp','DANet','PraNet','EMANet','DenseASPP','CaraNet','cswin',,'resT','banet','segbase','R2UNet','R2AttUNet','UNet3p'
    # models = ['CCNet','OCNet']
    # models = ['InfNet']
    # models = ["edgenext"]
    # models = ["parcnet"]
...
###Done
    ...
    models = ['MiniSeg',"nextvit",'poolformer','InfNet','UNet','volo','DeepLabv3','PSPNet','FCN','TransUNet','UNeXt','SwinUNet','SegNet','resnet','swinT']
    # models =  ['ctformer_t','ctformer_s','ctformer_b']

    # datasets = ['Inf', 'P9', 'P20', 'P1110', 'CVC', 'ISIC2018'] 
    datasets = ['P9', 'P20', 'P1110']
    ...
    for dataset in datasets:
        ...
        for model in models:
            ...
            if model =='nnUNet' or model =='UNet3p':
                args.batch_size = 4
            if model =='UNet++' or model =='UNet3p' or model == 'poolformer':
                args.batch_size = 16
