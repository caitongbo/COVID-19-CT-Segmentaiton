...
class NextViT(nn.Module):
    def __init__(self, stem_chs, depths, path_dropout, attn_drop=0, drop=0, num_classes=1000,
        ...
        # self.decode_head = SegHead(in_channels=[96, 256, 512, 1024], num_classes=2, in_index=[0, 1, 2, 3])

        # self.decode_head = APCHead(in_channels=head_dim[3], num_classes=nclass, in_index=3, channels=512)

        self.decode_head = ASPPHead(in_channels=1024, num_classes=2, in_index=3)
        ...
        self.auxiliary_head = FCNHead(num_convs=1, in_channels=512, num_classes=2, in_index=2, channels=256)

        self.edge_head = EdgeHead(in_channels=[96, 256, 512], in_index=[0, 1], channels=96)
    ...
    def forward(self, x):
        ...
        x0 = self.decode_head(output)
        if isinstance(x0, (list, tuple)):
            for out in x0:
                out = F.interpolate(out, size)
                outputs.append(out)
        else:
            x0 = F.interpolate(x0, size)
            outputs.append(x0)

        x1 = self.auxiliary_head(output)
        x1 = F.interpolate(x1, size)
        outputs.append(x1)

        edge = self.edge_head(output)
        edge = F.interpolate(edge, size)
        outputs.append(edge)
